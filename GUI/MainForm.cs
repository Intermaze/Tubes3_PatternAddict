        ...
        public MainForm()
        {
            ...
            EventHandler<EventArgs> search = async (object sender, EventArgs e) =>
            {
                ...
                if (isBM)
                {
                    ...
                    Console.WriteLine(wantToCompare);
                    (ans, path, percentage) = await Task.Run(() => Database.CompareFingerprintBM(wantToCompare, Converter.ImageToBin(selectedImagePath)));
                    Console.WriteLine(percentage);
                }
                else
                {
                    Console.WriteLine("Searching using KMP algorithm");
                    (ans, path, percentage) = await Task.Run(() => Database.CompareFingerprintKMP(wantToCompare, Converter.ImageToBin(selectedImagePath)));
                }
                watch.Stop();
            ...
            NotFound = CreateLabel("Tidak ada yang cocok", new Font(SystemFont.Bold, 10));
